...
def create_energy_system(regio_naam):
    # Create a new EnergySystem
    return EnergySystem(id=str(uuid.uuid4()), name="Vesta Resultaten PerPlanRegio: " + regio_naam)


def save_energy_system(es, filename):
    ...
    export_name = "output/%s.esdl" % filename
...
def add_strategy_to_energy_system(es, strategie_naam, regio_naam):

    instance = Instance(id=str(uuid.uuid4()), name=strategie_naam)

    instance.date = InstanceDate(date=EDate.from_string("2030-01-01"))
    instance.aggrType = AggrTypeEnum.PER_COMMODITY
    instance.area = Area(name=regio_naam)
    ...
    filename = "data/%s/PerPlanRegio_ESDL_%s.csv" % (strategie_naam, regio_naam)
...
#            area.asset.append(utilities)
            ...
            instance.area.area.append(area)
...
def main():
    regio_naam = "GooiEnVechtstreek"
    strategien = ["S0_Referentie", "S1a_AllElectric_lucht", "S1b_AllElectric_bodem"
                 , "S2a_restwarmte", "S2b_Geothermie-metcontour"
                 , "S2c_Geothermie-zondercontour", "S2d_BioWKK",
                  # "S3a_LT3030", "S3b_LT3070", "S3c_LT3050", "S3d_WKO", "S3e_TEO",
                  "S4_hwp_GG", "S5_HR_GG"]

    es = create_energy_system(regio_naam)

    for strategie_naam in list(strategien):
        add_strategy_to_energy_system(es, strategie_naam, regio_naam)
        print("ESDL-output generated for: ", regio_naam, strategie_naam)

    save_energy_system(es, "VESTA startanalyse resultaten voor " + regio_naam)
